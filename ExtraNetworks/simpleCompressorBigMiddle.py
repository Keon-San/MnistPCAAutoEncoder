...
class Network(nn.Module):
    def __init__(self, size_of_image, compressedSize):
        ...
        self.layer1 = nn.Linear(size_of_image*size_of_image, int((size_of_image*size_of_image)*.6))
        self.layer2 = nn.Linear(int((size_of_image*size_of_image)*.6), int(int((size_of_image*size_of_image)*.6)*.6))
        self.layer3 = nn.Linear(int(int((size_of_image*size_of_image)*.6)*.6), compressedSize)
        self.layer4 = nn.Linear(compressedSize, int(int((size_of_image*size_of_image)*.6)*.6))
        self.layer5 = nn.Linear(int(int((size_of_image*size_of_image)*.6)*.6), int((size_of_image*size_of_image)*.6))
        self.layer6 = nn.Linear(int((size_of_image*size_of_image)*.6), size_of_image*size_of_image)
...
net.load_state_dict(torch.load("simpleNetBigMiddle.state"))
learningRate = 0.001
...
netOptim.load_state_dict(torch.load("simpleNetBigMiddle.optim.state"))
...
def doSave():
    torch.save(net.state_dict(), "simpleNetBigMiddle.state")
    torch.save(netOptim.state_dict(), "simpleNetBigMiddle.optim.state")
